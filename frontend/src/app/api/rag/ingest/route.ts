export async function POST(req: Request) {
  ...
  const schema = z.object({ projectId: z.string().uuid(), title: z.string().min(1), text: z.string().min(1) });
  ...
  const { ingestPlainText } = await import("@/lib/rag/ingest");
  const res = await ingestPlainText(sb, parsed.data as any);
