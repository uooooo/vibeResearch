...
export async function startThemeMastra(input: ThemeStartInput) {
  ...
  const findCandidates = createStep({
    ...
    async execute({ inputData }: any) {
      ...
      // Best-effort scholar + RAG context to ground candidate generation (non-blocking if fails)
        ...
        if (q) {
          ...
          if ((process.env.USE_RAG || "0") === "1") {
            try {
              ...
              const { searchRag } = await import("@/lib/rag/search");
              const items = await searchRag(sb, { query: q, projectId: inputData?.projectId, limit: 5 });
              ragCtx = { count: items.length, top: items.slice(0, 3).map((s) => s.text.slice(0, 120)) };
            } catch {}
          }
        }
      } catch {}
